...
export class TemporalPeopleExtractor {
  ...
  private extractPeople(content: string, title: string): PersonMention[] {
    ...
    while ((match = namePattern.exec(content)) !== null) {
      ...
      if (!this.isCommonWord(potentialName) && !people.some((p) => p.name === potentialName)) {
        ...
        if (this.isPersonContext(context)) {
          people.push({
            name: potentialName,
            context,
  ...
  /**
   * Check if context suggests this is a person
   */
  private isPersonContext(context: string): boolean {
    ...
    return personIndicators.some((indicator) => contextLower.includes(indicator));
  }
