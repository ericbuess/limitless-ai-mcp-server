...
async function main() {
  ...
  try {
    ...
    const dates = ['2024-01-13', '2024-01-14', '2024-01-15', today];
    ...
    console.log(
      `  Hit Rate: ${((lifelogStats.hits / (lifelogStats.hits + lifelogStats.misses)) * 100).toFixed(2)}%`
    );
    ...
    console.log(
      `  Hit Rate: ${((searchStats.hits / (searchStats.hits + searchStats.misses)) * 100).toFixed(2)}%`
    );
    ...
    const weekDates = ['2024-01-09', '2024-01-10', '2024-01-11', '2024-01-12'];

    await Promise.all(weekDates.map((date) => client.listLifelogsByDate(date)));
